...
BEGIN
	...
	WHERE [Nome] IN ('João Silva', 'Maria Santos', 'Ricardo Sousa', 'Ana Oliveira', 'João Tomas')
	)
		BEGIN
	...
	ELSE
		BEGIN
			...
			('João Tomas', 1, 0);  -- Leitor com ultima requisição há mais de um ano
		END
END
GO
...
CREATE OR ALTER PROCEDURE Leitores_DesativarPorInfracoes
AS 
BEGIN
/*
Parte II - 9.
...
*/
UPDATE Leitores 
SET Ativo = 0 , Infracoes = 0
WHERE Leitores.Ativo <> 0 AND Leitores.Infracoes > 3
END
GO

CREATE OR ALTER PROCEDURE Leitores_Reativar
@ID_Leitor BIGINT
AS 
BEGIN
/*
...
SET 
    Ativo = 1
WHERE Leitores.Ativo <> 0 
  AND Leitores.ID_Leitor = @ID_Leitor;
END
GO
