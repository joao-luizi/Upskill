...
function isDeadSnake() {
  for (let i = 0; i < GS.snake.length; i++) {
    if (GS.snake.filter((s) => s.x === GS.snake[i].x && s.y === GS.snake[i].y).length !== 1)
      return true;
  }
  return false;
}
...
function UpdateSnakePosition() {
  ...
  if (GS.lastInput.x != -dx) head.x += GS.lastInput.x;
  else head.x += dx;
  if (GS.lastInput.y != -dy) head.y += GS.lastInput.y;
  else head.y += dy;
